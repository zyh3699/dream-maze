class Player {
  ...
  show(collisionMap) {
    ...
    if (collisionMap[interactY][interactX] === 6) {
      this.showMessage(
        "致幻蘑菇：当然可以食用了！现实里，一生你只有一次机会吃掉它，但是在这里，谁知道呢？"
      );
    }
    ...
    if (collisionMap[interactY][interactX] === 10) {
      if (localStorage.getItem("chapter4") === "true") {
        this.showMessage1(
          "永不放弃的编梦者...密码是1289，致敬你的勇气，请记住它继续完成后面的挑战吧。同时按住go就可以直接回到下一层梦境"
        );
        document.getElementById("mapp").style.textDecoration = "line-through";
      } else {
        this.showMessage(
          "宝藏箱：Pirate of Caribbean. 身为被诅咒之人，你似乎无法触碰到它。但是还是恭喜你找到宝藏。"
        );
        document.getElementById("mapp").style.textDecoration = "line-through";
      }
    }
    ...
    if (collisionMap[interactY][interactX] === 18) {
      if (localStorage.getItem("chapter4") === "true") {
        this.showMessage1("还记得曾经说你被诅咒的那个宝箱吗...");
      } else {
  ...
  interact(collisionMap) {
    ...
    if (
      collisionMap[interactY][interactX] === 2 &&
      this.dialogAde === 0 &&
      this.dialogKarl === 0
    ) {
    ...
    if (collisionMap[interactY][interactX] === 17 && this.dialogGhost === 0) {
      ...
      function handleOption(option) {
        ...
        if (option === "不，我现在没有空->") {
          ...
          document.getElementById("ghost").style.textDecoration =
            "line-through";
          function chuansong() {
